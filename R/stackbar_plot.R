...
#'
#' @examples
#' \dontrun{stackbar_plot(data = taxa, color_scheme = NULL, tax_cla = "genus",
...
# tools::showNonASCIIfile(file.path(dirname(rstudioapi::getActiveDocumentContext()$path), "stackbar_plot.R"))
stackbar_plot <- function(data, color_scheme = NULL, tax_cla, x_group = "group",
         ...
         filename = NULL, file_width = 16, file_height = 9)
{
  ...
  if (is.null(color_scheme)) {
    ...
    color_n <- max(1, length(unique(data[[tax_cla]])) - 1)
    color_scheme2 <- color_scheme[1:color_n]
    color_scheme2 <- c(color_scheme2, "#7f7f7f")
  } else {
  ...
  p <- ggplot2::ggplot() +
    ...
    ggplot2::geom_bar(data = data,
                      ggplot2::aes(
                        x = !!x_group,
                        weight = !!y_abun,
                        fill = forcats::fct_relevel(stringr::str_to_title(!!tax_cla), after = Inf,
                                                    "Others")),
                      position = bar_type,
                      width = bar_width) +
    ...
    ##
    ggplot2::theme_bw() +
    ggplot2::scale_fill_manual(values = color_scheme2) +
    ...
    ggplot2::theme(
      axis.text.x = ggplot2::element_text(angle = angle_x, hjust = angle_hjust, vjust = angle_vjust)  # 设置角度、水平和垂直对齐
    ) +
    ...
    ggplot2::theme(legend.text = ggplot2::element_text(
      face = "bold.italic", size = size_legend, color="black")) +
  ...
  ##
  cat("\033[32mstackbar: success!\033[0m\n")
