#' @title R6 Class: class_stackbar
#'
#' @description
#' The class_stackbar class encapsulates the species composition analysis
#' function `stackbar()` and its accompanying visualization function
#' `stackbar_plot()`, enabling convenient analysis.
#'
#' @return class_stackbar: otu, tax, metadata, data, chart
#' @export
#'
#' @importFrom R6 R6Class
class_stackbar <- R6::R6Class(
  "class_stackbar",
  ...
  public = list(
    ...
    #' @description
    #' The `plot()` function integrates the `stackbar()` and `stackbar_plot()`
    ...
    #' @examples
    #' \dontrun{
    #' a <- class_stackbar$new(otu, tax, metadata)
    #' a$plot()
    #' }
    #'
    ...
    )
    {
      ...
      # amplysis::stackbar_plot
      p <- amplysis::stackbar_plot(
        data = taxa,
