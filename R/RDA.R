...
#' @examples
#' \dontrun{
#' RDA(otu = otu, env = env, metadata = metadata, id_col = 1,
                ...
                replicate_method = "none")
{
  ...
  ## Format check
  # Check if the metadata dataframe contains "sample", "replicate"
  ...
  # Extract columns named "sample", "replicate", the value of argument 'group1', and the value of argument 'group2'
  ...
  if(!replicate_method %in% allowedMethods) {
    stop("Please enter the correct parameter for argument replicate_method:\n",
         "Process based on the `replicate` column in the `metadata` table, where samples with the same `replicate` value are considered replicate samples\n",
  ...
  if (replicate_method != "none") {
    ...
    metadata3 = metadata2 %>%
      # Keep the columns "replicate" and the column represented by group
...
When `Axis lengths` value < 3.0, choose RDA;\n\033[0m")
  ...
  ##
  # The vegan package rda function supports RDA analysis
  df_rda <- vegan::rda(t(otu6), env6, scale = T)


  ## Extract plotting data
  df_rda2 <- data.frame(df_rda$CCA$u[, 1:2], rownames(env6))                # Extract data
  colnames(df_rda2) = c("RDA1", "RDA2", "sample")                       # Rename columns
  df_rda2 <- dplyr::left_join(df_rda2, metadata3, by = c("sample" = "sample"))  # Merge plotting data with groups


  ## Unified the grouping column name in df_rda2 to "group"
  if (group %in% colnames(df_rda2)) {
    colnames(df_rda2)[colnames(df_rda2) == group] <- "group"
  }


  ##
  # Extract other data
  df_rda_score <- data.frame(df_rda$CCA$v[,1:2])            # Extract species scores
  df_rda_env <- as.data.frame(df_rda$CCA$biplot[,1:2])      # Extract environmental factor scores


  ##
  # Calculate axis label data (= axis eigenvalue / sum (eigenvalues of all axes))
  RDA1 = round(df_rda$CCA$eig[1] / sum(df_rda$CCA$eig) * 100, 2)
  RDA2 = round(df_rda$CCA$eig[2] / sum(df_rda$CCA$eig) * 100, 2)


  ## Environmental factor and community structure differences analysis: significance calculation
  # cca_sum <- summary(df_rda)  # Descriptive statistics
  # Test the significance of environmental factors (Monte Carlo permutation test)
  # df_permutest <- permutest(df_rda, permu = 999) # permu = 999 indicates the number of permutation cycles
  # Significance test for each environmental factor
  df_envfit <- vegan::envfit(df_rda, env6, permu = 999)
  # Data processing
  ...
  # Mark p < 0.05 as FALSE, p > 0.05 as TRUE, and use this data to plot a bar chart.
  cor_com[1:5,3] = cor_com[,3] > 0.05

  result <- NULL
  result <- list(
    data = df_rda2,        # RDA data
    RDA1 = RDA1,           # Axis eigenvalue 1
    RDA2 = RDA2,           # Axis eigenvalue 2
    score = df_rda_score,  # Extract species scores
    env = df_rda_env,      # Extract environmental factor scores
    cor_com = cor_com      # Environmental factor and community structure differences analysis: significance calculation
  )
